...
export class Explorer {
  ...
  private _bindTreeEvents(): void {
    ...
    this.treeView.on("open", (path: string) => {
      try {
        ...
        if (this.events["open_file"])
          this.events["open_file"](path, defaultApp);
      } catch (e: any) {
        if (window.AppUI)
    ...
    this.treeView.on("duplicate", async (path: string) => {
      try {
        ...
        this._emitHistory(
          "file_created",
          `User duplicated file: ${path} -> ${newPath}`,
        );
      } catch (e: any) {
        if (window.AppUI) window.AppUI.notify(e.message, "error");
      }
    ...
    this.treeView.on("move", async (srcPath: string, destPath: string) => {
      try {
        ...
        this._emitHistory(
          "file_moved",
          `User moved file: ${srcPath} -> ${destPath}`,
        );
      } catch (e: any) {
        if (window.AppUI) window.AppUI.notify(e.message, "error");
      }
    ...
    this.treeView.on("download", async (path: string) => {
      try {
        ...
        if (stat.kind === "file") {
          if (window.AppUI)
            window.AppUI.showLoading(`Downloading ${stat.name}...`);
        ...
        if (window.AppUI)
          window.AppUI.notify(`Download failed: ${e.message}`, "error");
      }
    });
  ...
  private async _downloadDirectoryAsZip(dirPath: string) {
    if (typeof JSZip === "undefined") {
      if (window.AppUI)
        window.AppUI.notify("System Error: JSZip library not loaded.", "error");
      return;
    }
    if (window.AppUI)
      window.AppUI.showLoading(`Compressing ${dirPath || "root"}...`);
    ...
    try {
      ...
      this._emitHistory(
        "project_exported",
        `User downloaded directory: ${dirPath}`,
      );
    } catch (e: any) {
  ...
  private _bindUploads(): void {
    ...
    if (this.els.INPUT_FILE) {
      this.els.INPUT_FILE.onchange = (e) => {
        const isFolder =
          this.els.INPUT_FILE?.hasAttribute("webkitdirectory") || false;
  ...
  private async _promptCreateRoot(type: "file" | "folder") {
    ...
    if (type === "folder") {
      ...
      try {
        ...
        this._emitHistory(
          "file_created",
          `User created empty file: ${fullPath}`,
        );
  ...
  private async _handleUploadAppend(
    e: Event,
    isFolder: boolean,
  ): Promise<void> {
  ...
  private _bindSidebarDnD(): void {
    ...
    sidebar.addEventListener("drop", async (e) => {
      ...
      for (let i = 0; i < items.length; i++) {
        const item =
          typeof items[i].webkitGetAsEntry === "function"
