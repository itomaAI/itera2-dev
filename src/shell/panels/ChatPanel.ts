...
export class ChatPanel {
  ...
  private _createStreamElement() {
    ...
    if (this.currentStreamContent && this.renderer) {
      this.currentStreamEl.innerHTML = this.renderer.formatStream(
        this.currentStreamContent,
      );
  ...
  finalizeStreaming() {
    if (this.currentStreamEl) {
      const header =
        this.currentStreamEl.parentElement!.querySelector("div:first-child");
  ...
  private _appendTurn(turn: Turn) {
    ...
    let baseClass =
      "relative group p-3 rounded-lg text-sm mb-2 border transition";
    ...
    header.className =
      "flex justify-between items-center mb-1 opacity-50 text-[10px] font-bold uppercase";
    ...
    if (typeof turn.content === "string") {
      if (
        role === "model" ||
        (role === "system" && turn.content.includes("<"))
      ) {
        if (this.renderer)
          body.innerHTML = this.renderer.formatStream(turn.content);
    ...
    if ((window as any).MathJax) {
      (window as any).MathJax.typesetPromise([body]).catch((e: any) =>
  ...
  private _renderArrayContent(
    ...
    contentArray.forEach((item) => {
      if (item.text) {
        const div = document.createElement("div");
        if (
          (role === "model" || item.text.trim().startsWith("<")) &&
          this.renderer
        ) {
  ...
  private async _renderMediaFromVfs(container: HTMLElement, mediaObj: any) {
    if (!this.vfs) {
      const div = document.createElement("div");
      div.className =
        "text-xs text-text-muted italic border border-border-main p-2 rounded mt-2";
