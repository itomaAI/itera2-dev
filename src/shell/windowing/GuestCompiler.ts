...
export class GuestCompiler {
  ...
  /**
   * 読み込めなかったパスの代わりに見せる Blob URL を作る。
   *
   * ★ ここで要求されたパスをそのまま返してはならない。
   *   配信先が SPA リライト（未知の URL すべてに index.html を 200 で返す）を行っていると、
   *   ブラウザは 404 の代わりに**親 OS の HTML** を受け取る。それが iframe の中で起動して
   *   「OS の中で OS が動く」状態になり、画面が壊れる（ミャク楽 Agent で実際に起きた / T-0196）。
   *   404 が 404 として返ってくることを当てにできないので、こちら側で閉じる。
   */
...
`;
  ...
  async compile(
    ...
    const entryUrl = await this._processFile(vfs, entryPath, pid, blobUrls, visited, '', args);
  ...
  private async _processFile(
    ...
    if (!basePath || basePath.match(/^https?:\/\//) || basePath.startsWith('data:')) {
      return requestPath;
    }
    ...
    if (isHtml) {
      ...
      htmlContent = await this._processHtmlDependencies(htmlContent, vfs, pid, blobUrls, visited, absPath);
      htmlContent = this._injectHtmlScripts(htmlContent, pid, search, hash, args);
      ...
      cssContent = await this._processCssDependencies(cssContent, vfs, pid, blobUrls, visited, absPath);
  ...
  private async _processHtmlDependencies(
    ...
    const processElements = async (selector: string, attr: string) => {
      ...
      for (const el of elements) {
        ...
        const newUrl = await this._processFile(vfs, val, pid, blobUrls, visited, currentDir);
  ...
  private async _processCssDependencies(
    ...
    for (let i = matches.length - 1; i >= 0; i--) {
      ...
      const newUrl = await this._processFile(vfs, relPath, pid, blobUrls, visited, currentDir);
  ...
  private _injectHtmlScripts(
    ...
    if (args) {
      const safeArgs = JSON.stringify(args).replace(/</g, '\\u003c').replace(/>/g, '\\u003e');
    ...
    html = html.replace('</body>', this._getScreenshotHelperCode(pid) + '\n</body>');
