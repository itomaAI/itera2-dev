...
export class ApiSettingsModal {
  ...
  private async _ensureInit() {
    if (!this.hasRendered && this.els.CONTAINER) {
      ...
      this.providers.forEach((provider) => {
        ...
        label.className = 'block text-2xs font-bold text-text-muted uppercase tracking-wider mb-1';
  ...
  private _loadValues() {
    ...
    this.providers.forEach((provider) => {
      ...
      if (provider.requiresUrl) {
        const urlInput = document.getElementById(`api-url-${provider.id}`) as HTMLInputElement;
