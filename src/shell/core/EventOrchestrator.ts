...
export class EventOrchestrator {
  ...
  private _bindExplorerEvents(): void {
    ...
    explorer.on("open_file", async (path: string, resolvedApp?: ResolvedApp) => {
      ...
      if (!resolvedApp) {
        ...
        if (resolvedApp.appId === "HostRunner") {
          ...
          await this.processManager.spawn(resolvedApp.appId, resolvedApp.appPath!, "foreground", false, { file: path }, currentUri);
        }
      }
    });
    ...
    explorer.on("history_event", (type: string, desc: string) => {
      ...
      const turn = this.history.append("system", lpml, { type: "event_log", trigger_llm: false });
  ...
  private _bindChatEvents(): void {
    ...
    chat.on("clear", async () => {
      if (await window.AppUI?.confirm("Clear chat history and media cache?")) {
        this.sessionManager.clearSession({ purgeMedia: true, triggerLlm: false });
      }
    });
  ...
  private _bindModalEvents(): void {
    ...
    this.desktop.modals.editor.on("save", async (path: string, content: string) => {
      try {
        await this.vfs.writeFile(USER_PRINCIPAL, path, content, { overwrite: true });
  ...
  private async _handleChatSend(text: string, attachments: File[], vfsReferences: string[]) {
    ...
    for (const path of vfsReferences) {
      content.push({ text: `<user_attachment path="${path}">[Existing VFS Path]</user_attachment>` });
    }
    ...
    for (const file of attachments) {
      ...
      try {
        if (isText) {
          await this.vfs.writeFile(USER_PRINCIPAL, path, data, { system: true, overwrite: true });
          content.push({ text: `<user_attachment name="${file.name}" path="${path}">\n${data}\n</user_attachment>` });
        } else {
          // バイナリは Blob のまま VFS の ContentStore (OPFS) に逃がす
          await this.vfs.writeFile(USER_PRINCIPAL, path, file, { system: true, overwrite: true });
          content.push({
            media: { path: path, mimeType: file.type || "application/octet-stream", metadata: {} }
          });
          content.push({ text: `<user_attachment path="${path}">[Binary File: ${file.name}]</user_attachment>` });
        }
      } catch (e: any) {
