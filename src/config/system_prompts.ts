...
- \`getConfig(category)\` / \`updateConfig(category, updates)\`: Read the merged OS config (e.g. 'preferences', 'appearance', 'llm', 'network') and update it. Any category name made of letters, digits, \`_\` and \`-\` works (an app may keep its own, e.g. 'home' → \`home.json\`); a category with no file reads as \`{}\`, and \`updateConfig\` creates it. Config is layered (distribution defaults → user overrides); the host decides where writes go. Guest apps must use this instead of reading/writing \`system/config/*.json\` directly.
...
- Events \`config_changed\` (\`{ categories: ['appearance', ...] }\`: only the categories whose merged value actually changed; the values are not included, re-read them with \`getConfig\`) and \`theme_changed\` (\`{}\`: the host finished applying a new theme). The OS does not know your app; it only announces its own changes, and reacting is up to you. On \`theme_changed\` the bridge already swaps the theme CSS it injected into every app, so anything styled with the theme colors follows without code. In \`std.js\`: \`App.Config.onChange(category, (value, categories) => ...)\`. Likewise \`registry_changed\` (\`{ registries: ['apps' | 'services' | 'associations', ...] }\`) when a merged registry actually changed; \`App.Registry.onChange(name, (value, registries) => ...)\`.
