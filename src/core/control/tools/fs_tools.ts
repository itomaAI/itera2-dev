...
export function registerFSTools(registry: ToolRegistry): void {
  ...
  registry.registerSystemTool(setId, setName, {
    ...
    impl: async (params: any, context: { vfs: VfsService }) => {
      ...
      const msg = await context.vfs.writeFile(
        AGENT_PRINCIPAL,
        params.path,
        content,
      ...
      if (/<{4,}SEARCH/.test(content)) {
        ...
        while ((startMatch = startRegex.exec(content)) !== null) {
          ...
          const patternStr = content
            .substring(contentStart, midMatch.index)
            .replace(/(?:\r?\n)$/, "");
          ...
          const replaceStr = content
            .substring(replaceStart, endMatch.index)
            .replace(/(?:\r?\n)$/, "");
        ...
        if (blocks.length === 0) {
          throw new Error(
            "Invalid edit block format. Ensure you use SEARCH, ====, and >>>> markers correctly.",
          );
        }

        const isRegex = params.regex === "true";
        let currentFileContent = await vfs.readFile(
          AGENT_PRINCIPAL,
          params.path,
        );
        let replaceCount = 0;
        ...
        for (let i = 0; i < blocks.length; i++) {
          ...
          if (!regex.test(currentFileContent)) {
            throw new Error(
              `Search pattern not found for block ${i + 1}. No changes were made to the file.`,
            );
          }
          ...
          if (newContent === currentFileContent) {
            throw new Error(
              `Replacement resulted in no change for block ${i + 1}. No changes were made to the file.`,
            );
          }
        ...
        const blockMsg =
          replaceCount > 1
            ? `${replaceCount} blocks updated`
            : "Content replaced";
        return { log: blockMsg, ui: `✏️ Replaced content in ${params.path}` };
      }

      throw new Error(
        "Invalid <edit_file> content. Use SEARCH markers or specify 'mode' attribute.",
      );
    },
  });
      ...
      const formatOutput = (fileList: any[]) => {
        ...
        return fileList
          .map((f) => {
            ...
            const dateStr = new Date(f.updatedAt)
              .toISOString()
              .slice(0, 19)
              .replace("T", " ");
  ...
  registry.registerSystemTool(setId, setName, {
    ...
    impl: async (params: any, context: { vfs: VfsService }) => {
      const msg = await context.vfs.rename(
        AGENT_PRINCIPAL,
        params.path,
        params.new_path,
      );
  ...
  registry.registerSystemTool(setId, setName, {
    ...
    impl: async (params: any, context: { vfs: VfsService }) => {
      const msg = await context.vfs.copyFile(
        AGENT_PRINCIPAL,
        params.path,
        params.new_path,
      );
