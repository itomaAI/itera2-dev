...
export class VfsService {
  ...
  private _checkNodePermission(principal: Principal, nodeId: string | null, action: PermissionType): void {
  ...
  private _getDefaultAcl(principal: Principal, parentId: string | null): AccessControlList {
    if (parentId) {
  ...
  private _filterTreeByPermission(principal: Principal, nodes: TreeNode[]): TreeNode[] {
    ...
    return nodes.filter((node) => {
      ...
      if (!vfsNode || !this._hasPermission(principal, vfsNode, 'read')) return false;
      if (node.children) {
  ...
  getUsage(principal: Principal): {
    ...
    for (const node of this.nodeStore.getAllNodes()) {
      if (node.kind === 'file' && node.meta.size && this._hasPermission(principal, node, 'read')) {
        used += node.meta.size;
      }
    }
