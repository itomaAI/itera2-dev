...
|---|---|---|
| `vfs_index/chunk_*` の並列ロード | `files.list` のフラット検索（`q="trashed=false"`, `pageSize=1000`） | 6,000 件で約 7 通信 |
...
4. `changes.list` が使えない環境 → `buildIndexFlat()` へ退避（それでも約 7 通信）
...
`test/gdrive_index_test.html` が `derivePathMap()` / `isExcluded()` を検証する
（31 ケース）。実装をコピーせず、`gdrive_sync.html` の `<script>` 本体を
読み込んで評価するため、**実装が変われば試験も追随する**。

対象は入れ子パスの導出、フォルダ改名時の追随、到達不能エントリの破棄、
ゴミ箱の親を持つ子の除去、同名重複の決定性、allowlist 除外（接頭辞の
誤爆を含む）、部分木マウント。

さらに**変異試験**で試験の有効性自体を確認している
（パス連結・新旧比較・同着判定をそれぞれ壊すと、対応するケースが落ちる）。