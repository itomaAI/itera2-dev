...
バイト列は Drive にあるのでこの経路ではデータ消失に至らないが
（**ただしリモートが到達可能であり続ける場合に限る**。後述「コールドスタートは
ローカルの実体を破棄しない」を必ず併せて読むこと。実際に 104 ファイルを失っている）、
...
## コールドスタートはローカルの実体を破棄しない

上の対策は「アンカーが空になる」という**引き金**を減らすものであって、
コールドスタート分岐そのものを安全にしたわけではなかった。
引き金は他にもある（接続先フォルダの切り替え、同期状態のリセット、
マウント先の変更）。そして分岐の中身は破壊的だった。

**2026-08-12 07:49、これで実際に 104 ファイルを失った。**

```
07:29:41  MUTATE  ""     isMountPoint          マウント先を drive/ から VFS ルートへ変更
07:49:02  MUTATE  ×104   gdrive_sync_daemon    changed: hash, syncState, contentRef
```

マウント先の変更でアンカーが空になり、次サイクルがコールドスタート分岐に入って、
当時の接続先フォルダに存在したパスのローカル実体を捨てた。
そのフォルダは後に到達不能となり（Drive のゴミ箱にも残っていない）、
**内容は復元不能**である。

> したがって前節の「バイト列は Drive にあるのでデータ消失ではない」は、
> **リモートの実体が到達可能であり続ける限りにおいてのみ**正しい。
> `createStub()` はその保証が無いまま `contentRef` を外していた。

一度この状態になると自力復帰もできない。Push 分岐は `MetaOS.fs.read()` で
実体を読もうとし、スタブなので `onFetchContent` に落ち、
fileId が無いため失敗する。実際に 104 件は 4 日間アンカーに入らないままだった。

**規則: コールドスタートでスタブ化してよいのは「ローカルに実体が無い」場合だけ。**

| ローカルの状態 | 動作 |
|---|---|
| 実体がある（`l.hash` が非 null） | **スタブ化しない。** ローカルを温存し、`anchor.hash = null` を記録する |
| スタブ（`l.hash === null`） | 従来どおり `createStub` |
| 不在 | 従来どおり `createStub`（新規端末の初回同期はこの経路） |

温存した場合は次サイクルで `localChanged` が立ち、Push 分岐がリモートへ送る。
リモートを上書きしうるが、**Drive 側は版管理とゴミ箱で復旧できる一方、
ローカルのバイト列は復旧できない。** この非対称性に従ってローカルを優先する。

新規端末ではローカルが存在しないため挙動は変わらない。
**変わるのは「実体を捨てようとしていた場合」だけである。**
...
2. **削除を実行する前に現物を 1 件確認する**（`verifyRemoteAlive()`）。
   ...
   削除は稀なので追加コストは無視できる。
3. **索引の変更をディスクへ残す**（`indexDirty` + 定期フラッシュ）。
   上記 1 はメモリ上の `index.byId` を書き換えるだけなので、
   保存しないまま再起動すると前回保存以降のアップロード分が索引から欠落する。

#### `verifyRemoteAlive()` は 3 値である（fail-closed）

**「不在」と「判定不能」を混同してはならない。**
当初この関数は 2 値（生存 / それ以外）で、認証エラー以外の例外をすべて
「リモートに無い」へ倒していた。つまり Drive の一時的な 5xx・レート制限・
通信瞬断で、**ローカルのファイルが恒久削除されうる**状態だった。
安全網の内部に、安全網を無効化する穴が開いていたことになる。

| 戻り値 | 条件 | 呼び出し側の動作 |
|---|---|---|
| `'alive'` | 200 かつ `trashed !== true` | 索引へ復元して何もしない（gap 修復） |
| `'gone'` | 404、または `trashed === true` | **ここでのみ**ローカルを削除する |
| `'unknown'` | 5xx / 通信断 / 解釈できない 200 / `id` 不明 | 何もしない。次サイクルで再判定 |

判別には `driveFetch()` が例外へ付与する `err.status` を使う。
メッセージ文字列の解析は脆いので採らない。

`'unknown'` が続く限りリモートで本当に削除されたファイルはローカルに残り続けるが、
**ユーザーのファイルを誤って消すことに比べれば軽微**である。

#### 索引の永続化はダーティフラグ方式

1 件ごとに保存すると数百 KB の書き込みが初回同期中に数千回発生する。
アンカーと同じく `ANCHOR_FLUSH_MS`（10 秒）間隔でフラッシュし、
サイクル末尾と中断時（`TOKEN_EXPIRED` 捕捉部）にも書き出す。

**部分的な索引を保存してはならない**という原則とは矛盾しない。
これらのフラッシュ点はすべて `reconcile()` のループ内にあり、
その時点の索引は `buildIndexFlat()` が全ページ取得を終えた完全な集合だからである。
